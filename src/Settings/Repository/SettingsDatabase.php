...
namespace BulkGate\PrestaSms\Settings\Repository;
...
use BulkGate\Plugin\{Structure\Collection, Settings\Helpers, Settings\Repository, Settings\Repository\Entity, Strict};
use BulkGate\PrestaSms\Database\Connection;

class SettingsDatabase implements Repository\Settings
{
	use Strict;
	...
	public function load(string $scope): Collection
	{
		...
		$result = $this->db->execute(
            "SELECT * FROM `{$this->db->table('bulkgate_module')}` WHERE `scope` = ? AND `synchronize_flag` != ? ORDER BY `order`",
            [
                $scope,
                'delete'
            ]
	...
	{
		$this->db->execute(
			"DELETE FROM `{$this->db->table('bulkgate_module')}` WHERE `scope` = ? AND `key` = ?",
            [
                $scope,
                $key
            ]
