...
namespace BulkGate\PrestaSms\Event\Repository;
...
use BulkGate\Plugin\{Strict, Structure\Collection, Event\Repository, Event\Repository\Entity};
use BulkGate\PrestaSms\Database\Connection;
use function implode;

class AsynchronousDatabase implements Repository\Asynchronous
{
	use Strict;
	...
	{
		$this->db->execute(
			"DELETE FROM `{$this->db->table('bulkgate_module')}` WHERE `scope` = 'asynchronous' AND `key` = ?",
            [
                $key
            ]
		);
